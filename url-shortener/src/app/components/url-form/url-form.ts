...
export class UrlFormComponent {
  ...
  encurtar(): void {
    ...
    const shortUrlEndpoint = `${environment.apiUrl}/shortener`;
    const payload: ShortenerRequest = {
      originalUrl: this.url
    };

    const subscription = this.http.post<ShortenerResponse>(shortUrlEndpoint, payload).subscribe({
      next: (urlResponse) => {
